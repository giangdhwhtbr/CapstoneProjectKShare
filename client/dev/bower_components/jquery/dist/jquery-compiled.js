...
// Prioritize by case sensitivity in case script pseudos are added with uppercase letters
...
if(!elemData){return;} // Caller can pass in an object of script data in lieu of the handler
...
name==="width"?1:0,val=0;for(;i<4;i+=2){ // Both box interface exclude margin, so add it if we want it
...
delete data.finish;});}});jQuery.each(["toggle","show","hide"],function(i,name){var cssFn=jQuery.fn[name];jQuery.fn[name]=function(speed,easing,callback){return speed==null||typeof speed==="boolean"?cssFn.apply(this,arguments):this.animate(genFx(name,true),speed,easing,callback);};}); // Generate shortcuts for script animations
...
rlocalProtocol=/^(?:about|app|app-storage|.+-extension|file|res|widget):$/,rnoContent=/^(?:GET|HEAD)$/,rprotocol=/^\/\//, /* Prefilters
	 * 1) They are useful to introduce script dataTypes (see ajax/jsonp.script for an example)
	 * 2) These are called:
...
// you can add your own script options here if
...
for(i in s.headers){jqXHR.setRequestHeader(i,s.headers[i]);} // Allow script headers/mimetypes and early abort
...
if(support.cors||xhrSupported&&!options.crossDomain){return {send:function send(headers,complete){var i,xhr=options.xhr();xhr.open(options.type,options.url,options.async,options.username,options.password); // Apply script fields if provided
